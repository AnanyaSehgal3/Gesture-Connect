...
class Application:
    ...
    def __init__(self):
        ...
        self.root = tk.Tk()
        self.root.title("Gesture Connect")
        self.root.protocol('WM_DELETE_WINDOW', self.destructor)
        self.root.geometry("1300x700")
        # --- Gradient Background ---
        self.root.configure(bg="#121212")

        self.canvas_bg = tk.Canvas(self.root, width=1300, height=700, highlightthickness=0)
        ...
        def draw_gradient(canvas, color1, color2):
            ...
            for i in range(700):  # vertical gradient lines
                nr = int(r1 + (r_ratio * i))
                ng = int(g1 + (g_ratio * i))
                nb = int(b1 + (b_ratio * i))
                hex_color = "#%04x%04x%04x" % (nr, ng, nb)
                canvas.create_line(0, i, 1300, i, fill=hex_color)

        draw_gradient(self.canvas_bg, "#3e1d5b", "#121212")

        # Styles
        bg_color = "#1a1a1a"
        hi="#3e1d5b"
        label_bg = "#1f1f1f"  # Simulated transparency
        text_fg = "#FFFFFF"
        accent_fg = "#BB86FC"
        btn_bg = "#292929"
        btn_fg = "#FFFFFF"
        font_main = ("Montserrat", 24, "bold")
        font_label = ("Montserrat", 18, "bold")
        font_btn = ("Montserrat", 12, "bold")

        # Title
        self.T = tk.Label(self.root, text="Gesture Connect", font=("Montserrat", 36, "bold"),
                          fg=text_fg, bg=hi)
        self.T.place(x=450, y=20)

        # Panels
        self.panel = tk.Label(self.root, bg=label_bg)
        self.panel.place(x=100, y=115, width=480, height=300)

        self.panel2 = tk.Label(self.root, bg=label_bg)
        self.panel2.place(x=700, y=115, width=400, height=300)

        # Character display
        self.T1 = tk.Label(self.root, text="Character:", font=font_label, fg=text_fg, bg=bg_color)
        self.T1.place(x=50, y=440)
        self.panel3 = tk.Label(self.root,  font=font_label, fg=accent_fg, bg=label_bg)
        self.panel3.place(x=250, y=440)


        # Sentence display
        self.T3 = tk.Label(self.root, text="Sentence:", font=font_label, fg=text_fg, bg=bg_color)
        self.T3.place(x=50, y=520)
        self.panel5 = tk.Label(self.root, font=font_label, fg=accent_fg, bg=label_bg)
        self.panel5.place(x=250, y=520)

        # Suggestions
        self.T4 = tk.Label(self.root, text="Suggestions:", font=font_label, fg="#FF80AB", bg=bg_color)
        self.T4.place(x=50, y=580)

        # Buttons
        self.b1 = tk.Button(self.root, text="Option 1", font=font_btn, bg=btn_bg, fg=btn_fg, bd=0, relief="flat",
                            padx=20, pady=10)
        self.b1.place(x=300, y=580)

        self.b2 = tk.Button(self.root, text="Option 2", font=font_btn, bg=btn_bg, fg=btn_fg, bd=0, relief="flat",
                            padx=20, pady=10)
        self.b2.place(x=450, y=580)

        self.b3 = tk.Button(self.root, text="Option 3", font=font_btn, bg=btn_bg, fg=btn_fg, bd=0, relief="flat",
                            padx=20, pady=10)
        self.b3.place(x=600, y=580)

        self.b4 = tk.Button(self.root, text="Option 4", font=font_btn, bg=btn_bg, fg=btn_fg, bd=0, relief="flat",
                            padx=20, pady=10)
        self.b4.place(x=750, y=580)

        self.speak = tk.Button(self.root, text="Speak", font=("Montserrat", 16, "bold"), command=self.speak_fun,
                               bg="#6200EE", fg="#FFFFFF", padx=20, pady=10)
        self.speak.place(x=700, y=450)

        self.clear = tk.Button(self.root, text="Clear", font=("Montserrat", 16, "bold"), command=self.clear_fun,
                               bg="#03DAC6", fg="#000000", padx=20, pady=10)
        self.clear.place(x=900, y=450)
